...
if(Serial.available() > 0)
  {
   ...
   if(letter == '1')
    ...
    sprintf(ch,"%02X",readByteAt(csPIN,i));  //calls the read fuction in for loop
  ...
  else if(letter == '2')
 {
    ...
    Serial.readBytes(readData,512); //Reads the serial port
    for (int i=0;i<256;i++){
    ...
    char ch[30];
    for (uint16_t i=0;i<256;i++){       //how deep to write
      sprintf(ch,writeByteAt(csPIN, i, dump[i])); //calls the write fuction in for loop
      }
delay(500);
